...
import { CloudCiteStore } from '../state/cloudcite-store';
...
class CloudCiteBibliography extends HTMLElement {
    ...
    constructor(...args) {
        ...
        this._citationStyle = CloudCiteStore.getActiveProject().style.value;
        this._locale = CloudCiteStore.getLocale().value;
        this._format = [];
        this._citationHTML = [];
    ...
    async generatePreview() {
        if (CloudCiteStore.getActiveProject().citations.length > 0) {
            let cslObject = simplifyObject(this._citationData);
            for (let i = 0; i < CloudCiteStore.getActiveProject().citations.length; i++) {
                cslObject[CloudCiteStore.getActiveProject().citations[i].id] = CloudCiteStore.getActiveProject().citations[i];
            }
                        ...
                        <div style=${{marginTop: '10px', backgroundColor: '#ffffff', border: '1px solid #e0e0e0', padding: '20px', borderRadius: '5px', textAlign: 'left', fontWeight: 'normal !important'}}>
                            <div class="csl-bib-body" style=${{lineHeight: this._format.linespacing, marginLeft: `${this._format.hangingindent}em`, textIndent: `-${this._format.hangingindent}em`}}>
                                ${this._citationHTML.map((cslEntry, index) =>
                                    ...
                                    <button id="copyCitationButton${index}" style=${{borderRadius: '5px'}}>Copy Citation</button>`
                                )}
                            </div>
                        </div>
                    </div>`;
    ...
    render() {
        return this.html`
        <div>
            ${CloudCiteStore.getActiveProject().citations.length == 0 ? this.html`<p>This bibliography looks a little empty. You can create a new citation from the menu.</p>`: ``}
            ${{
                any: this.generatePreview(),
                placeholder: CloudCiteStore.getActiveProject().citations.length > 0 ? this.html`<p>Loading...</p>`: ``
            }}
        </div>`;
    }
}
